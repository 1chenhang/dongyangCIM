...
export default class Draw {
  ...
  drawRectangle () {
    ...
    this.handler.setInputAction((click) => {
      ...
      /** 面实例对象 */
      const polygons = this.viewer.entities.add(rectangleOptions)
      this.handler.setInputAction((move) => {
        const cartesian = this.viewer.camera.pickEllipsoid(move.endPosition, this.viewer.scene.globe.ellipsoid)
        const cartographic = Cesium.Cartographic.fromCartesian(cartesian, this.viewer.scene.globe.ellipsoid, new Cesium.Cartographic())
        const lng = Cesium.Math.toDegrees(cartographic.longitude)
        const lat = Cesium.Math.toDegrees(cartographic.latitude)
    ...
    this.handler.setInputAction(() => {
      this.handler.destroy()
      this.infoDetail.rectangle.push({
        id: id,
        position: westSouthEastNorth
      })
    }, Cesium.ScreenSpaceEventType.RIGHT_CLICK)
  }
  ...
  drawCircle () {
    ...
    this.handler.setInputAction((click) => {
      ...
      this.handler.setInputAction((move) => {
        boundaryPositions = []
        const cartesian2 = this.viewer.camera.pickEllipsoid(move.endPosition, this.viewer.scene.globe.ellipsoid)
        if (Cesium.defined(cartesian2)) {
          radius = Cesium.Cartesian3.distance(cartesian, cartesian2)
          // 计算几何体圆的轮廓
          const circleOutlineGeometryCreate = Cesium.CircleOutlineGeometry.createGeometry(new Cesium.CircleOutlineGeometry({
            center: new Cesium.Cartesian3.fromDegrees(...lngLat, 0),
            radius: radius || 1,
            granularity: 0.01
          }))
          // 获取几何体圆的边界坐标
          const positions = [].slice.call(circleOutlineGeometryCreate.attributes.position.values)
  ...
  drawLine (callBack) {
    ...
    this.handler.setInputAction((movement) => {
      ...
      window.tooltip.showAt(windowPosition, '<p>点击绘制线,右击绘制结束</p>')
    ...
    this.handler.setInputAction((movement) => {
      ...
      this.infoDetail.ids.push(_polygonEntity._id)
      this.handler.destroy()
      callBack(positions)
    }, Cesium.ScreenSpaceEventType.RIGHT_CLICK)
  }
  ...
  removeAll () {
    this.infoDetail.ids.forEach(item => {
