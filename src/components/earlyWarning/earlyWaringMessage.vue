<template>
    <div class="tableWrapper">
      <el-table ref="reportTable" height="175" class="table-with-header-bg" :data="tableList" stripe
        ...
        style="width: 100%;">
        <el-table-column prop="alarmContent" label="预警信息"  />
        <!-- <el-table-column prop="deviceName" label="摄像头名称" />
        <el-table-column prop="townName" label="属地" />
  ...
  <script setup>
  import { ref, onMounted,nextTick,toRefs,defineProps } from 'vue'
  ...
  const props = defineProps({
    //子组件接收父组件传递过来的值
    earlyWaringList: Array,
  })

const {earlyWaringList} =toRefs(props)
tableList.value = earlyWaringList.value;
  ...
  const handleClick = async (row) =>{
    ...
    $bus.emit('closeEarlyMessageDilog')
  }
