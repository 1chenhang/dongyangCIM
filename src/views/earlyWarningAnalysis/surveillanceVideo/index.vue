...
const addCustomTextTrack = () => {
  ...
  // 动态添加多条字幕内容，按时间顺序显示
  const cues = [
    new VTTCue(0, 3, `预警信息: ${analySisData.value.alarmContent}`),
    new VTTCue(3, 6, `预警位置: ${analySisData.value.townName}`),
    new VTTCue(6, 8, `预警位置: ${analySisData.value.townName}`),
    new VTTCue(8, 10, `预警种类: ${analySisData.value.alarmTypeName}`),
    new VTTCue(10, 12, `预警地经纬度: ${analySisData.value.alertPlotLon} ${analySisData.value.alertPlotLat}`),
  ];

  cues.forEach(cue => {
    // 自定义字幕的显示位置
    cue.line = 100; // 垂直位置 (0 表示顶部)
    cue.position = 52; // 水平位置 (100 表示右边)
