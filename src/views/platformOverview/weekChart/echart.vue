<template>
  <div class="wrapper">
    <p class="tip">单位(个）</p>
    <div style="width: 100%; height: 180px" ref="main"></div>
  </div>
...
const { near14DaysWarn } = toRefs(props)
const XName = ref([])
const data1 = ref([])
const data2 = ref([
  {
...
watch(near14DaysWarn, (newValue, oldValue) => {
  const near14DaysWarn = newValue;
  let arr = [];
  let arr2 = [];
  let arr3 = [];
  if (near14DaysWarn.length > 0) {
    near14DaysWarn.forEach(item => {
      arr.push(item.date)
      arr2.push(item.count)
      arr3.push([item.date, item.count])
    })
    XName.value = arr;
    data1.value = arr2;
    data2.value[0].coords = arr3;
    init()
  }
