...
import { countByHandleType} from '@/api/camera.js'
...
const { code, msg, data } = await countByHandleType(params);
if (code === '200') {
  let {fast_process,regular_rocess} = data;
  yjlxData.value  = [
    {
      name: '常规处置',
      value: regular_rocess
    },
    {
      name: '快速处置',
      value: fast_process
...
const setIntervals = (myChart) => {
  ...
  timed.value = setInterval(() => {
    console.log('--在调用')
    index++
    if (index > dataList.value.length - 1) index = 0
    myChart.dispatchAction({
      type: 'downplay', // 取消高亮
      seriesIndex: 0
    })
