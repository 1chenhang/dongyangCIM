...
import { ref, onMounted, toRefs, defineProps, watch, nextTick } from 'vue'
import { getFaultCountByDay } from '@/api/statics.js'
const main = ref()
// 数据
const XName = ref([])
const dataNum = ref([])
...
const getList = async () => {
  const { data, code } = await getFaultCountByDay(timeQuery.value)
  if (code !== '200') return
  XName.value = []
  dataNum.value = []
  dataList.value[0].coords = []
  console.log(dataList.value, '--dataList.value')
  data.forEach(item => {
    XName.value.push(item.date)
    dataNum.value.push(item.fault_count)
    dataList.value[0].coords.push([item.date, item.fault_count])
  })
}
    ...
    series: [
      {
          ...
          normal: {
            color: new echarts.graphic.LinearGradient(
              ...
              [
                {
                  offset: 0,
                  color: 'rgba(242, 46, 88, 0.32)'
                },
                {
                  offset: 0.8,
                  color: ' rgba(242, 46, 88, 0)'
                }
              ],
              false
            ),
        ...
        lineStyle: {
          ...
          color: {
            ...
            y2: 1,
            colorStops: [{
              offset: 0, color: '#F22E58' // 起始颜色
            }, {
              offset: 1, color: '#F22E58' // 结束颜色
            }],
