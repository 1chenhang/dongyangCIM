...
import { getStaticsJudState } from '@/api/statics.js'
import 'echarts-gl'
...
const getList = async () => {
  const { data, code } = await getStaticsJudState(timeQuery.value)
  if (code === '200' && data) {
    dataList.value[0].value = data.effective_count
    dataList.value[1].value = data.invalid_count
  }
}
const init = () => {
...
.tipBox {
  ...
  height: 137px;
  padding: 0 33px;
  background: linear-gradient(180deg, rgba(3, 119, 255, 0.16) 0%, rgba(19, 130, 233, 0) 100%);
  display: flex;
  flex-wrap: wrap;
