...
import { getAlarmOtherTypeCountByDate} from '@/api/camera.js'
...
const getWarninfoData = async (params) => {
  ...
  const { code, msg, data } = await getAlarmOtherTypeCountByDate(params);
  if (code === '200') {
...
const init = async () => {
  ...
  const colorList = ['rgba(65, 241, 141,  0.9)', 'rgba(72,209,204,  0.9)', 'rgba(255, 208, 118,  0.9)', 'rgba(255,99,71, 0.9)', 'rgba(124,252,0,  0.9)'
  ,'	#DC143C','#00FFFF','	#00008B','#008B8B','#B8860B','#A9A9A9','#006400','#FF8C00','#00CED1','#FF1493','#00BFFF']
